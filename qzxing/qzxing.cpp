...
QString QZXing::decodeImage(QImage image, int maxWidth, int maxHeight, bool smoothTransformation)
{
    ...
    if(image.isNull())
    {
        emit decodingError("Not an Image");
    ...
    {
        //qDebug() << "[QZXING ERROR]" << e.what();
        emit decodingError(e.what());
        emit decodingFinished(false);
       processingTime = -1;
       return "";
    }
...
QString QZXing::qmlDecode(QString itemID)
{
    /*
    QUrl imageUrl(itemID);
        QQmlEngine* engine = QQmlEngine::contextForObject(this)->engine();
        QQmlImageProviderBase* imageProviderBase = engine->imageProvider( imageUrl.host());

        QQuickImageProvider* imageProvider = new QQuickImageProvider(imageProviderBase->imageType(),imageProviderBase->flags());
